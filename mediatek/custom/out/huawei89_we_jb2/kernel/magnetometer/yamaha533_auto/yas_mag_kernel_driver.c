...
#define DEBUG_RW 0
/*----------------------------------------------------------------------------*/
#define YAMAHA530_DEV_NAME         "yamaha533"
...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
extern int yamaha_orientation_sensor_init(void);
extern void yamaha_orientation_sensor_exit(void);
extern  void yamaha_test_init_clint(struct i2c_client * client);
static int yamaha530_init_flag =0; // 0<==>OK -1 <==> fail  //f00184246
static const struct i2c_device_id yamaha530_i2c_id[] = {{YAMAHA530_DEV_NAME,0},{}};
static struct i2c_board_info __initdata i2c_yamaha530={ I2C_BOARD_INFO("yamaha533", (0x5c>>1))};
...
static int yamaha533_local_init(void) ;
static int yamaha533_remove(void);
static short g_calibration_value = 0;
...
static struct sensor_init_info yamaha533_init_info = {
		.name = "yamaha533",
		.init = yamaha533_local_init,
		.uninit =yamaha533_remove,
	
};

static int yamaha533_test_pcb()
{
	...
	int x,  y1,y2,dir;
	 ...
	 int sx,  sy ;
     struct yas_pcb_test test ;

    ret = yas_pcb_test_init(&test);
	printk("yas_pcb_test_init ret=%d\n",ret);
    if(ret!=0)
   { 
       printk("yas_pcb_test_init  error ret=%d\n",ret);
       return ret;
   }

  ret= test.power_on_and_device_check(&ID);
 	printk("power_on_and_device_check ret=%d\n",ret);
   if(ret!=0)
   { 
      printk("power_on_and_device_check ret=%d\n",ret);
       return ret;
    ...
    ret= test.initialization();
	printk("initialization ret=%d\n",ret);
    if(ret!=0)
   { 
       printk("test.initialization ret=%d\n",ret);
       return ret;
   }
...
//	MSE_FUN(f);
	...
	switch (cmd)
	{
		...
		case ECS_IOCTL_APP_GET_CAL:
		    ...
		    if (copy_to_user(argp, &g_calibration_value, sizeof(g_calibration_value))) 
			{
		     	return -EFAULT;
	        }
	       	break;
...
static int yamaha530_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	temp = i2c_smbus_read_byte_data(client, 0x80);
	printk("yamaha533_i2c_probe,temp =0x%x,\n",temp);
    if(0x02!=temp)
    {
    	g_calibration_value = 0;//COMPASS_MMI_CONTECT_ERROR
        printk("yamaha533_i2c_probe,fail deviceID =0x%x,COMPASS_MMI_CONTECT_ERROR\n",temp);
        goto err;
    }

    yamaha_test_init_clint(client);
    
    result=yamaha533_test_pcb();
    if(0==result)
    {
    	g_calibration_value = 1;//COMPASS_MMI_SUCESS
        printk("yamaha530_i2c_probe,test ok COMPASS_MMI_SUCESS!");
    }
    else
    {
    	g_calibration_value = -1;//COMPASS_MMI_FAILED
        printk("yamaha530_i2c_probe,test error!COMPASS_MMI_FAILED\n");
        goto err;
    }
    data->hw = yamaha533_auto_get_cust_mag_hw();
	...
	if(yamaha530_create_attr(&yamaha533_init_info.platform_diver_addr->driver))
	{
...
static int yamaha530_i2c_remove(struct i2c_client *client)
{
	...
	if(data != NULL)
	{
		...
		if(err = yamaha530_delete_attr(&yamaha533_init_info.platform_diver_addr->driver))
	    {
...
{
       int status =0;
       struct mag_hw *hw = yamaha533_auto_get_cust_mag_hw();
	...
	if(i2c_add_driver(&yamaha530_i2c_driver))
	{
       ...
       if(-1 == yamaha530_init_flag)
        {
        	MSE_ERR("yamaha530_i2c_driver i2c probe fail!\n");
            return -1;
        }

       status =yamaha_orientation_sensor_init();
       if ( 0!=status)
       {
           printk("failed to register yamaha orientation");
       }
	return 0;
}
/*----------------------------------------------------------------------------*/
static int yamaha533_remove(void)
{
    ...
    yamaha_orientation_sensor_exit();
	return 0;
}
/*----------------------------------------------------------------------------*/
/*static struct platform_driver yamaha_sensor_driver = {
...
/*----------------------------------------------------------------------------*/
static int __init yamaha533_init(void)
{
	MSE_FUN();
	i2c_register_board_info(YAMAHA_533_I2C_NUM, &i2c_yamaha530, 1);
	...
	hwmsen_msensor_add(&yamaha533_init_info);
	return 0;    
}
/*----------------------------------------------------------------------------*/
static void __exit yamaha533_exit(void)
{
	MSE_FUN();
...
module_init(yamaha533_init);
module_exit(yamaha533_exit);
